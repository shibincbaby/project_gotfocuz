...
// $date = date('Y/m/d h:i:s', time());
 //echo $date;
	...
	if(isset($_POST['submit']))
    {
		...
		$sql4="SELECT * FROM `tbl_booking` WHERE `s_id`='$studioid' AND `date_occ`='$datime'";
		...
		VALUES ('$studioid', '$date', '$place', '$landmark', '$datime', '$tim24', '$lid')";
		//echo $sql;
		$result=mysqli_query($connection,$sql);
		$cookie_value=0;
	$status=setcookie($cookie_name, $cookie_value, time()+3600);
			//echo "<script>if(confirm('studio booked for this date!!!!')){document.location.href='web/user_home.php'}else{document.location.href='index.php'};</script>";
