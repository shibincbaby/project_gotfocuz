<?php
$user=$_SESSION['username']; //you can fetch username here
$db=new mysqli('localhost','root','','db_studio');
if($db->connect_errno){
echo $db->connect_error;
}
$pull="select * from tbl_user  where `email`='$user'";
...
&& in_array($extension, $allowedExts))
{
	...
	else
	{
		...
		unlink("upload/" . $_FILES["file"]["name"]);
			...
			$query="update tbl_user set img_url='$url' where `email`='$user'";
...
echo "<img src='upload/$pics[img_url]' alt='profile picture' width='80 height='64'   class='doubleborder'/></div>";
?>
